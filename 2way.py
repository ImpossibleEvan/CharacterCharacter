...
# --- Main Execution ---
def main():
    # If no command-line arguments are given, run an interactive setup.
    if len(sys.argv) == 1:
        try:
            choice = input("Run as (s)erver or (c)lient? ").lower().strip()
            if choice.startswith('s'):
                port_str = input("Enter port to listen on [5000]: ")
                port = int(port_str) if port_str else 5000
                server = Server(port)
                server.run()
            elif choice.startswith('c'):
                host = input("Enter server host to connect to: ")
                port_str = input(f"Enter server port [5000]: ")
                port = int(port_str) if port_str else 5000
                sock = start_client(host, port)
                if sock:
                    run_client_game(sock)
            else:
                print("Invalid choice. Please run again and choose 's' or 'c'.")
...
if __name__ == "__main__":
    try:
        ...
        try:
            ...
            # This can fail in non-interactive environments
            print("Exiting without waiting for input.")